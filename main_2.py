        ...
        for x in range(triangle_height, center_x - square_side // 2, int(self.dot_spacing)):
            ...
            for y in row_y_positions:
                if bit_index < len(self.bits):
                    if self.is_inside_hexagon(x, y, image_size) and not self.is_inside_square(x, y, image_size, square_side):
                    ...
                    mirrored_x, mirrored_y = self.mirror_coordinates(x, y, center_x, center_y)
                    if self.is_inside_hexagon(mirrored_x, mirrored_y, image_size) and not self.is_inside_square(mirrored_x, mirrored_y, image_size, square_side):
